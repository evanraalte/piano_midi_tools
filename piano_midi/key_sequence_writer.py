...
class KeySequenceWriter:
    ...
    def press(self, key_idx: int, frame_num: int) -> None:
        typer.echo(f"frame {frame_num}: Pressing key {key_idx}")
        self.pressed_keys[key_idx] = frame_num

    def release(self, key_idx: int, frame_num: int) -> None:
        if key_idx in self.pressed_keys:
            typer.echo(f"frame {frame_num}: Releasing key {key_idx}, it was pressed for {frame_num - self.pressed_keys[key_idx]} frames")
